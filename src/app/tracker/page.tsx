...
export default function TrackerPage() {
  ...
  const progress = Math.round(((tracker.stage - 1) / (STEPS.length - 1)) * 100);
  const isLastStage = tracker.stage >= STEPS.length;
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr]">
        ...
        <Card className="animate-slide-up">
          ...
          <Stepper
            steps={STEPS}
            currentStep={tracker.stage}
            onStepClick={(step) => setTrackerStage(step)}
          />
        </Card>
        ...
        <div className="flex flex-col gap-6">
          <Card className="animate-slide-up" style={{ animationDelay: "0.1s" }}>
            <div className="flex items-start gap-4 mb-4">
              <span className="text-3xl">{STAGE_ICONS[tracker.stage - 1]}</span>
              <div>
                <p className="text-xs text-[var(--trust-blue)] font-semibold uppercase tracking-wider mb-1">
                  Stage {tracker.stage} of {STEPS.length}
                </p>
                <h3 className="text-xl font-bold">{STEPS[tracker.stage - 1]}</h3>
              </div>
            </div>
            <p className="text-[var(--text-bright)] text-sm leading-relaxed mb-5">
              {DESCRIPTIONS[tracker.stage - 1]}
            </p>
            {tracker.stage > 1 && (
          ...
          {tracker.stage > 1 && (
            <Card className="animate-slide-up" style={{ animationDelay: "0.15s" }}>
              ...
              <div className="flex flex-col gap-2.5">
                {STEPS.slice(0, tracker.stage - 1).map((step, i) => (
          ...
          <div className="flex flex-col gap-3 animate-slide-up" style={{ animationDelay: "0.2s" }}>
            {!isLastStage && (
              <Button
                ...
                size="lg"
                className="w-full"
                onClick={() => setTrackerStage(Math.min(tracker.stage + 1, STEPS.length))}
              >
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-[var(--bg-surface)]/90 backdrop-blur-md border-t border-[var(--border)]">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <span className="text-xs text-[var(--text-secondary)] whitespace-nowrap">
            {tracker.stage}/{STEPS.length} stages
          </span>
        </div>
      </div>
    </div>
